...
Auckland Transport (AT) [publishes daily and monthly bike count data from counters around Auckland](https://at.govt.nz/cycling-walking/research-monitoring/monthly-cycle-monitoring/#data) now available under CC BY 4.0 license.

```{r tweet,echo=FALSE}
blogdown::shortcode('tweet', '1074441737127391232')
```
...
Finally create an arima model using the auto.arima function from the [forecast](https://cran.r-project.org/web/packages/forecast/index.html) R package to see what the growth in cycling could look like.
...
The bike count data is interesting data to look at, especially the time series components and forecast. Once the coordinates are available in the AT datasets, further geospatial analysis can be performed through delving into the [sf](https://cran.r-project.org/web/packages/sf/index.html) package and using other data.
